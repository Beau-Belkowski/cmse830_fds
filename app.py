...
with tabs[3]:
    ...
    st.write("##### The reason I chose to use random forest and gradient boosted regressor models is because these models are good at handling data with a lot of variables. The data I have has a lot of variables and many of them are not highly correlated to each other which is another aspect of the data that these types of models handle well. The results of the Random Forest and Gradient Boosted Regressor models were very similar so I will show their results here. The random forest model found that penalty yards had a 0.003993% affect on predicting wins in the model. Similarly the Gradient Boosted Regressor model found that penalty yards had a 0.003115% affect on predicting wins in the model. These percent values indicated what number of trees that the model created, included penalty yards on them. Since these numbers are so low they indicate that the penalty yards has little to no affect on predicting wins. To be clear, these models were not mean't to be fitted or be the best model they could be. The point of these models was to see how penalty yards would contribute to predicting wins when all the variables are considered.")
